...
interface AdminGameSetupProps {
  company?: string;
  projectId?: string;
}

export function AdminGameSetup({ company, projectId }: AdminGameSetupProps) {
  ...
  const [players, setPlayers] = useState<AdminPlayerInput[]>([
    { firstName: "", lastName: "", email: "" },
    { firstName: "", lastName: "", email: "" },
    { firstName: "", lastName: "", email: "" },
  ]);
  const [invites, setInvites] = useState<
    { name: string; email?: string; link: string; token: string }[] | null
  >(null);

  const computedQuota = useMemo(() => {
    if (talents.length === 0) return 0;
    if (pickMode === "percentage")
      return Math.max(1, Math.ceil((talents.length * pickValue) / 100));
  ...
  function onDownloadTemplate() {
    ...
    const rows = [
      headers,
      ["Jane Doe", "Sales", "EMEA", "N-2", "G8", "3y", "1y", "A", "H"],
      [
        "John Smith",
        "Engineering",
        "PLATFORM",
    ...
    try {
      const base =
        typeof window !== "undefined"
          ...
          : "";
      const { invites } = await createGame({
        sessionName,
        pickMode,
        pickValue,
        rawRows,
        ...
        <CardContent className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="grid gap-1.5">
            ...
            <p className="text-xs text-muted-foreground">
              Default 15%. Current quota from pool: <b>{computedQuota}</b>
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="grid gap-3">
          ...
          <p className="text-sm text-muted-foreground">
            Detected talents: {talents.length}. Showing all columns below.
          </p>
          {rawRows.length > 0 && (
        ...
        <CardContent className="grid gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Input
              ...
              value={playersCount}
              onChange={(e) =>
                setPlayersCount(Math.max(1, Number(e.target.value) || 1))
              }
              className="w-28"
            />
          </div>
                  ...
                  <TR key={idx}>
                    ...
                    <TD>
                      <Input
                        ...
                        onChange={(e) => {
                          const n = [...players];
                          n[idx] = { ...n[idx], firstName: e.target.value };
                          setPlayers(n);
                        }}
                        placeholder="Name"
                      />
                    ...
                    <TD>
                      <Input
                        ...
                        onChange={(e) => {
                          const n = [...players];
                          n[idx] = { ...n[idx], lastName: e.target.value };
                          setPlayers(n);
                        }}
                        placeholder="Surname"
                      />
          ...
          <div className="flex gap-2">
            <Button
              onClick={onSetupGame}
              disabled={!sessionName || rawRows.length === 0 || players.length === 0}
            >
              Setup the Game
            </Button>
          </div>
